...
/*
	Library hanya akan memberikan akses untuk
	2 method d bawah, selain itu tidak ada
	akses untuk tahu proses retrieve data
	cuaca
*/
...
/*
	Format JSON yang akan menjadi Response
	daripada REST API Server Third Party
*/
type Weather struct {
	ID   int    `json:"id"`
...
type CurrentWeatherData struct {
	// APIKey sebagai pengenal untuk akses
	// karena client harus lebih dahulu
	// terdaftar
	APIkey string
}

/*
	akan berperan sebagai mock service
	karena kita tidak akan integrasi dgn
	REST API Server yang sebenarnya
*/
func getMockData() io.Reader {
	...
	clouds","icon":"04n"}],
	"base":"stations","main":{"temp":303.56,"pressure":1
...
func (o *CurrentWeatherData) doRequest(uri string) (weather *Weather, err error) {
	...
	if resp.StatusCode != 200 {
		...
		err = fmt.Errorf("Status code was %d, aborting. Error messagewas:\n%s\n",
...
func (c *CurrentWeatherData)GetByGeoCoordinates(lat, lon float32)
...
func (c *CurrentWeatherData) GetByCityAndCountryCode(city, countryCode string) 
				(weather *Weather, err error) {

	return c.doRequest(
			fmt.Sprintf("http://api.openweathermap.org/data/2.5/weather?
				lat=%f&lon=%f&APPID=%s", city, countryCode, c.APIkey) )
}